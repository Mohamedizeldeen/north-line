    ...
    <form method="POST" action="{{ route('admin.blog-posts.update', $blogPost) }}" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            <textarea name="excerpt" rows="2"
                      class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">{{ old('excerpt', $blogPost->excerpt) }}</textarea>
        ...
        <div>
            ...
            <textarea name="content" rows="12" required
                      class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">{{ old('content', $blogPost->content) }}</textarea>
        ...
        <div>
            ...
            @if($blogPost->featured_image)
                <div class="mb-2">
                    <img src="{{ Storage::url($blogPost->featured_image) }}" alt="" class="w-32 h-20 object-cover rounded-lg">
                </div>
            @endif
        ...
        <div class="flex items-center gap-2">
            ...
            <input type="checkbox" name="is_published" value="1" id="is_published" {{ old('is_published', $blogPost->is_published) ? 'checked' : '' }}
